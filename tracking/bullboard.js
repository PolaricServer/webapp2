...
pol.tracking.BullBoard = function()
{
    ...
    var showGroup = {
        view: function(vn) {
            return m("table", vn.attrs.msgs.map(function(x) {
                return m("tr", [ m("td", x.sender), m("td", m("table", x.bulls.map(function(y) {
    ...
    t.server.pubsub.subscribe("bullboard", function(x) {
        getGroups();
        getMessages();
        getAnn();
    });  
    ...
    function getGroups() {
        t.server.GET("/bullboard/groups", "", function(x) { 
            groups = JSON.parse(x);
            groups.unshift('APRS');
            m.redraw();
        } );
    ...
    function getMessages() {
        ...
        if (g == 'APRS')
            g = '_B_'; 
        t.server.GET("/bullboard/"+g+"/messages", "", function(x) { 
    ...
    function getAnn() {
        t.server.GET("/bullboard/_A_/messages", "", function(x) { 
...
ol.inherits(pol.tracking.BullBoard, pol.core.Widget);
